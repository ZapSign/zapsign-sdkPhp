...
$deParaName = new DeParaTemplate('{{NOME COMPLETO}}', 'Signer Name Async');
...
$docFromTemplate = (new DocFromTemplateBuilder())
    ...
    ->withLang('pt-br')
    ->withSignerName('test signer name template async')
...
try {
    $docResponse = (new DocRequests($apiToken))->createDocFromTemplateAsync($docFromTemplate);
    echo $docResponse;
} catch (Exception $e) {
    echo $e->getMessage();
}
